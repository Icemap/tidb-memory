...
from models import ChatSession, AppState
...
def initialize_services():
    ...
    if 'session_manager' not in st.session_state:
        st.session_state.session_manager = SessionManager(llm_service=st.session_state.llm_service)
        ...
        if len(session.messages) > 0:
            with st.spinner("Generating session summary..."):
                ...
                if summary:
                    ...
                    st.warning(f"Session {session.session_id} closed but summary generation failed")
        else:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                try:
                    ...
                    # Generate response
                    response = st.session_state.llm_service.generate_response_sync(conversation_history)
